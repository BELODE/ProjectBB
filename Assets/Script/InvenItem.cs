    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        if (num != 10)
        {
            ...
            if (eventData.pointerEnter == null && items.name != null)
            {
                ...
                RaycastHit2D hit = Physics2D.Raycast(dir, Vector2.zero, 0f, 1 << LayerMask.NameToLayer("Obstacle"));

                if (hit.collider != null && GameObject.Find("Player").GetComponent<PlayerMove>().target != null && GameObject.Find("Player").GetComponent<PlayerMove>().target.tag == "Obstacle")
                {
                    hit.transform.GetComponent<ObstacleInven>().ItemIn(transform.parent.parent.GetComponent<Inventory>().dragItem.GetComponent<InvenItem>().items.code);
                }
                else if(hit.collider==null||(hit.collider!=null&&GameObject.Find("Player").GetComponent<PlayerMove>().target==null)|| (hit.collider != null && GameObject.Find("Player").GetComponent<PlayerMove>().target != null&&GameObject.Find("Player").GetComponent<PlayerMove>().target.tag!="Obstacle"))
                {
